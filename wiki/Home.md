# 🎬 PyMkvPropEdit — Wiki

Bienvenue dans le wiki de **PyMkvPropEdit** !
Welcome to the **PyMkvPropEdit** wiki!

---

## 📚 Pages disponibles / Available pages

| Page | Description FR | Description EN |
|------|---------------|----------------|
| [🏠 Home](Home) | Page d'accueil | Home page |
| [⚡ Installation](Installation) | Guide d'installation | Installation guide |
| [🚀 Démarrage rapide](Quick-Start) | Premiers pas | Getting started |
| [🚀 Batch Pro](Batch-Pro) | Renommage auto + métadonnées Kodi | Auto-rename + Kodi metadata |
| [🔑 Clés API](API-Keys) | Obtenir les clés TVDB / TMDB | Get TVDB / TMDB keys |
| [🎵 Audio Sync](Audio-Sync) | Synchronisation audio FFT | FFT audio sync guide |
| [🗂️ Onglets](Tabs-Overview) | Description de chaque onglet | Each tab explained |
| [⚙️ Options & Paramètres](Options-Settings) | Configuration | Configuration guide |
| [❓ FAQ](FAQ) | Questions fréquentes | Frequently asked questions |

---

## 🔗 Liens rapides / Quick links

- [📦 Télécharger la dernière version / Download latest](https://github.com/Crysisjim/PyMkvPropEdit/releases/latest)
- [🐛 Signaler un bug / Report a bug](https://github.com/Crysisjim/PyMkvPropEdit/issues)
- [📄 Changelog complet](https://github.com/Crysisjim/PyMkvPropEdit#changelog)

---

## 🌐 Langue / Language

Ce wiki est disponible en **français** et **anglais**.
This wiki is available in **French** and **English**.

> Chaque page contient les deux langues. / Each page contains both languages.
